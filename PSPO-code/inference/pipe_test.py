...
pipe = pipeline("text-generation", model=model, tokenizer=tokenizer,device_map="auto",max_new_tokens=512,num_beams=5)
...
def run(data_path,out_path,task):
    ...
    with open(out_path, mode='a+', encoding='utf-8') as fout:
        i = 0
        for out in tqdm(pipe(dataset, batch_size=batch_size, return_full_text=False), total=len(dataset),desc=task):
            res = dict()
            res['id'] = str(i)
